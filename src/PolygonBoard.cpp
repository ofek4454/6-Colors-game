...
void PolygonBoard::create() {
    srand(time(NULL));
    const float total_width = (PAD_WIDTH*1.5+2)*NUM_OF_COLS;
    const float starting_x = WINDOW_WIDTH/2 - total_width/2;

    const float total_height =(PAD_WIDTH*1.5+2) * NUM_OF_ROWS;
    const float starting_y = WINDOW_HEIGHT / 2 - total_height / 2;

    int i = 0;
    float x = starting_x, y = starting_y;
    for (int row = 0; row < NUM_OF_ROWS; row++, y += PAD_WIDTH*1.5 +2) {
        for (int col = 0; col < NUM_OF_COLS; col++, x += PAD_WIDTH*1.5 +2, i++) {
