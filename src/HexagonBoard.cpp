...
void HexagonBoard::create() {
    ...
    for (int row = 0; row < NUM_OF_ROWS; row++, y += PAD_WIDTH * 1.5) {
        for (int col = 0; col < NUM_OF_COLS; col++, x += (PAD_WIDTH * 1.5) + 4) {
            ...
            auto pad = std::make_shared<Pad>(i, 6,Colors(rand), sf::Vector2f(x, y));
...
void HexagonBoard::attachNeighbors() {
...
void HexagonBoard::printBoardObject(sf::RenderWindow &window) {
